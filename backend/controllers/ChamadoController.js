...
module.exports = class ChamadoController {
    static async criarChamado(req, res) {
        ...
        try {
            ...
            if (!mesa) {
                return res.status(404).json({ message: "Botão não cadastrado. Mesa não encontrada." });
            }
            ...
            const chamadoPendente = await Chamado.findOne({ mesa: mesa._id, status: 'pendente' });
            if (chamadoPendente) {
                return res.status(409).json({ message: "Esta mesa já possui um chamado pendente." });
            }
    ...
    static async aceitarChamado(req, res) {
        ...
        try {
            ...
            if (!chamado || chamado.empresa.toString() !== req.user.empresa.toString()) {
                return res.status(404).json({ message: "Chamado não encontrado." });
            }

            if (chamado.status !== 'pendente') {
                return res.status(409).json({ message: "Este chamado já foi atendido." });
            }
            ...
            res.status(500).json({ message: "Erro ao aceitar chamado.", error: error.message });
        }
    }
    ...
    static async resolverChamado(req, res) {
        ...
        try {
            const chamado = await Chamado.findOne({ _id: chamadoId, garcom: garcomId });
            if (!chamado) {
                return res.status(404).json({ message: "Chamado não encontrado ou não pertence a você." });
            }
            ...
            res.status(500).json({ message: "Erro ao resolver chamado.", error: error.message });
        }
    }
    ...
    static async listarPendentes(req, res) {
        ...
        try {
            const chamadosPendentes = await Chamado
                .find({ empresa: empresaId, status: 'pendente' })
                .sort({ timestamp_chamado: 1 })
                .populate('mesa', 'numero');

            res.status(200).json({ chamados: chamadosPendentes });
        } catch (error) {
            res.status(500).json({ message: "Erro ao buscar chamados pendentes.", error: error.message });
        }
    }
}